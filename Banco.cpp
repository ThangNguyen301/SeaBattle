...
int Banco::ktraWin()
{
	bool playerWin = true, botWin = false;
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 10; j++) {
			if (Mang2[i][j].thuyen == 1) { // Vẫn còn tàu của bot
				playerWin = false;
			}
			if (Mang1[i][j].thuyen == 1) { // Vẫn còn tàu của người chơi
				botWin = false;
			}
		}
	}
	...
	if (playerWin) {
		MessageBoxW(NULL, _T("You win!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
		return 1; // Người chơi thắng
	}
	if (botWin) {
		MessageBoxW(NULL, _T("Steve wins!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
		return 2; // Bot thắng
	}
	return 0;
}
...
int Banco::ktraWin(int mi, int mj)
{
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 10; j++) {
			...
			if (Mang1[mi][mj].thuyen == 1 && Mang1[mi][mj].bcolor != 2) {
				kq = 1;
				//MessageBoxW(NULL, _T("You win!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
				break;
			}
			if (Mang2[mi][mj].thuyen == 1 && Mang2[mi][mj].bcolor != 2); {
				kq = 0;
				//MessageBoxW(NULL, _T("You win!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
				break;
			}
	...
	if (kq == 1) {
		MessageBoxW(NULL, _T("Bot wins!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
		return 0;
	}
	if (kq == 0) {
		MessageBoxW(NULL, _T("You win!"), _T("Game Over"), MB_OK | MB_ICONINFORMATION);
...
/*
bool Banco::checkWin(bool isPlayer)
{
	auto& Mang = isPlayer ? Mang1 : Mang2;
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 10; j++) {
			if (Mang[i][j].thuyen == 1) {
				return false;
			}
		}
	}
